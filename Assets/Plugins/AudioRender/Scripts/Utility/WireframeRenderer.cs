...
namespace AudioRender
{
    public class WireframeRenderer : MonoBehaviour
    {
        ...
        [Header("Select rendering mode:\nAudio: Send graphics as audio for an oscilloscope\nEmulator: Open an oscilloscope emulator\nGfx: Use shader in Unity to simulate an oscilloscope")]
        ...
        [Header("X scale of the rendering. (Audio only)")]
        ...
        private void Start()
        {
            ...
            for (int i = 0; i < args.Length; i++)
            {
                ...
                if (args[i] == "-emulator")
                {
                    renderDeviceType = RenderDeviceType.Emulator;
                    Debug.Log("Use emulator");
                }
                if (args[i] == "-gfx")
                {
                    renderDeviceType = RenderDeviceType.Gfx;
                    Debug.Log("Use gfx render");
                }
                if (args[i] == "-scaleX")
                {
                    if (args.Length > i)
            ...
            if (renderDeviceType == RenderDeviceType.Emulator)
            {
            ...
            else
            {
                ...
                {
                    renderDevice = gameObject.AddComponent<AudioRender.GfxRenderDevice>();
                }
